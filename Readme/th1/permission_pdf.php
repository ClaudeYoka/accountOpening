...
$did = intval($_GET['permissionid']);
$sql = "SELECT tblpermission.id as lid, tblemployees.FirstName, tblemployees.LastName, tblemployees.Department, tblpermission.ToDate, tblpermission.FromDate, tblpermission.PostingDate, tblpermission.requested_days, tblpermission.requested_hours, tblpermission.RegRemarks, tblpermission.HodRemarks 
        FROM tblpermission JOIN tblemployees ON tblpermission.empid = tblemployees.emp_id WHERE tblpermission.id='$did'";
...
$num_days_requested = $row['requested_days'];
$hours_requested = $row['requested_hours'];
...
<body>
    <div class="header">
        ...
        Internal User Only
        <div class="title-box">
            <h1>FICHE DE DEMANDE DE PERMISSION</h1>
        </div>
    </div>
    ...
    <div class="content">
        ...
        <div class="section">
            ...
            <p class="form-label">DATE DE DEPART :</p> ' . $start_date . '<br>
            <p class="form-label">DATE DE REPRISE :</p> ' . $end_date . '<br>
            <p class="form-label">HEURE DEMANDES :    ' . $hours_requested . '</p> <br>
            <p class="form-label">DECISION DU HEAD : </p> ' . ($stats2 == 1 ? 'Approuvée' : ($stats2 == 2 ? 'non Aprouvée' : 'En cours')) . '<br>
            <p class="form-label">DECISION DES RESSOURCES HUMAINES :</p> '. ($stats == 1 ? 'Approuvée' : ($stats == 2 ? 'non Aprouvée' : 'En cours')) .  '<br>
        </div>
