...
if ($result && mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($chequier_info && $chequier_info['requested']) {
            ...
            // Fetch last status change date
            $status_q = mysqli_query($conn, "SELECT changed_at FROM chequier_status WHERE request_id = " . intval($row['id']) . " ORDER BY changed_at DESC LIMIT 1");
