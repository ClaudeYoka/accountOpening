...
$query = "SELECT tc.id, tc.account_number, tc.firstname AS customer_name, tc.branch_code, tc.type_compte, tc.etabliss AS quantity, tc.date_enregistrement AS created_at, COALESCE(tb.DepartmentName, tc.branch_code) AS agency_name, COALESCE(cs.status, tc.access, 'encours') AS current_status
          ...
          WHERE LOWER(REPLACE(COALESCE(cs.status, tc.access, 'encours'), ' ', '')) COLLATE utf8mb4_general_ci = 'livré'
...
<body>
        ...
        <div class="page-header">
            <div class="title">
                <h2 class="h3 mb-0">Demandes livrées en agence</h2>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">
                            <a href="dashboard_chequier">Dashboard chéquier</a>
                        </li>
                        <li class="breadcrumb-item active">Demandes livrées</li>
                    </ol>
                </nav>
            </div>
        </div>
        <div class="card-box mb-30"><div class="pd-20">
        <p><strong><?php echo count($requests); ?></strong> demande(s) livrée(s) en agence.</p>
        <div class="table-responsive"><table class="data-table table hover nowrap">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Compte</th>
        ...
        <tr>
            ...
            <td><span class="badge" style="background:#6f42c1;color:#fff;">Livré</span></td>
