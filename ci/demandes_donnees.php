...
$query = "SELECT tc.id, tc.account_number, tc.firstname AS customer_name, tc.branch_code, tc.type_compte, tc.etabliss AS quantity, tc.date_enregistrement AS created_at, COALESCE(tb.DepartmentName, tc.branch_code) AS agency_name, COALESCE(cs.status, tc.access, 'encours') AS current_status
          ...
          WHERE LOWER(REPLACE(COALESCE(cs.status, tc.access, 'encours'), ' ', '')) COLLATE utf8mb4_general_ci = 'donné'
...
<div class="page-header">
    <div class="title">
        <h2 class="h3 mb-0">Demandes Données</h2>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item">
                    <a href="index">Dashboard</a>
                </li>
                <li class="breadcrumb-item active">Demandes Données</li>
            </ol>
        </nav>
    </div>
</div>
...
<p>
    <strong><?php echo count($requests); ?></strong> demande(s) donnée(s).
</p>
    <div class="table-responsive">
            ...
            <tbody>
                ...
                <tr>
                    <td><?php echo $index + 1; ?></td><td><?php echo htmlspecialchars($request['account_number']); ?></td>
                    ...
                    <td><span class="badge" style="background:#28a745;color:#fff;">Donné</span></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
