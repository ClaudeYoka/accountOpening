...
if (isset($_POST["update_image"])) {
	...
	if(!empty($image)){
		...
		set_flash_message('Veuillez sélectionner une image pour la mettre à jour.', 'error');
		headers('Location: change_password.php');
		exit;
	}
    ...
    if ($result) {
        set_flash_message('Photo de profil modifiée avec succès.', 'success');
        header('Location: staff_profile.php');
        exit;
    } else {
        die(mysqli_error($conn));
